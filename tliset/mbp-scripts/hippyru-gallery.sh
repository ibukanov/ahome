...
if [[ ${#list[@]} -eq 0 ]]; then
    echo "Не найдено ни одной картинки в папке $(pwd)"
    exit 0
fi
...
for file in "${list[@]}" ; do
  ...
  printf '[%d/%d] %s -> %s\n' $counter ${#list[@]} "$file" "$convert_name"
	  #convert -auto-orient -filter Lanczos -resize x900 "$file" -quality 96 "thumbnails/$convert_name"
	  convert -auto-orient -resize x900 "$file" -quality 96 "thumbnails/$convert_name"
  let counter+=1
done
